...
export class ResponseService {
  ...
  async setMetadata(definitions: MetadataDefinition[]) {
    this.signLab.post<any>('api/response/metadata', definitions, { provideToken: true });
  }
  ...
  async getResponses(): Promise<Response[]> {
    return this.signLab.get<Response[]>('api/response/', { provideToken: true });
  }
  ...
  async uploadCSV(file: File): Promise<SaveAttempt> {
    ...
    return this.signLab.post<SaveAttempt>('api/response/upload/csv', form, { provideToken: true });
  }
  ...
  async uploadZIP(file: File): Promise<SaveAttempt> {
    ...
    return this.signLab.post<SaveAttempt>('api/response/upload/zip', form, { provideToken: true });
  }
  ...
  async addTag(tag: Tag, isTraining: boolean): Promise<void> {
    if (isTraining) {
      return this.signLab.post<any>('api/tag/completeTraining', tag, { provideToken: true });
    } else {
      return this.signLab.post<any>('api/tag/complete', tag, { provideToken: true });
    }
  }
