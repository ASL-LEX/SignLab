...
import { ResponseStudy, ResponseStudyDocument } from '../schemas/responsestudy.schema';
import { Response } from '../schemas/response.schema';


@Injectable()
export class ResponseStudyService {
  constructor(@InjectModel(ResponseStudy.name)
  ...
  async getAndMarkTagged(study: Study): Promise<ResponseStudy | null> {
    ...
    const responseStudy = await this.responseStudyModel.findOneAndUpdate(query, update)
      .populate('response')
      .populate('study')
      .exec();
   ...
   */
  async createResponseStudies(responses: Response[], study: Study): Promise<void> {
  ...
  async getResponseStudies(study: Study): Promise<ResponseStudy[]> {
    return await this.responseStudyModel
  ...
  async find(response: Response, study: Study) {
    return await this.responseStudyModel.findOne({
   ...
   *                      study.
   */
  async changePartOfStudy(responseStudy: ResponseStudy, isPartOfStudy: boolean): Promise<void> {
    await this.responseStudyModel.findOneAndUpdate({
  ...
  async markTraining(studyID: string, responseIDs: string[]) {
    await Promise.all(responseIDs.map(async (responseID) => {
